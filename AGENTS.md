...
- Env: `uv sync` (add `--extra serve` for FastAPI + the TypeSafe SDK; on Apple Silicon it also pulls `mlx-lm` for the MLX backend, `--extra mlx` alone for library use). `kev` is a real package (setuptools, installed
  ...
  group carries pytest, matplotlib and `modal==1.5.5`. On Windows torch resolves from the cu126 index (`[tool.uv.sources]`, marker-guarded to
  `sys_platform == 'win32'`; the PyPI win_amd64 wheel is CPU-only, so Linux and the Modal image are unchanged); README troubleshooting has the rest.
